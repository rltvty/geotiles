...
use crate::hexasphere::shape_theory::{calculate_distance_classes, predict_unique_shapes};
...
pub fn generate_unique_shapes_direct(subdivision_level: u32) -> Vec<TileShape> {
    ...
    for class in distance_classes.iter().skip(1) {
        // Skip pentagon class
...
fn create_pentagon_shape() -> TileShape {
    ...
    let vertices: Vec<Point> = (0..5)
        .map(|i| {
            ...
            Point::new(radius * angle.cos(), 0.0, radius * angle.sin())
        })
        .collect();
...
fn create_hexagon_shape_from_config(config: &TileConfig) -> TileShape {
    ...
    for i in 0..6 {
        ...
        vertices.push(Point::new(r * angle.cos(), 0.0, r * angle.sin()));
    }
...
fn create_shape_variation(base_shape: &TileShape, variation_index: usize) -> TileShape {
    ...
    for (i, vertex) in base_shape.vertices.iter().enumerate() {
        ...
        vertices.push(Point::new(vertex.x * factor, vertex.y, vertex.z * factor));
    }
...
mod tests {
    ...
    fn test_shape_placement_rules() {
        ...
        let total_instances: usize = (0..shapes.len()).map(|i| rules.instance_count(i)).sum();
