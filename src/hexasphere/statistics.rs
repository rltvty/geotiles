...
mod tests {
    ...
    fn test_hexagon_stats_size_relationships() {
        ...
        assert!(edge_to_radius_ratio > 0.5 && edge_to_radius_ratio < 2.0, 
            "Edge/radius ratio should be reasonable: {}", edge_to_radius_ratio);
    }
    ...
    fn test_hexagon_stats_total_tiles() {
        ...
        assert_eq!(stats.total_hexagons + stats.total_pentagons, hexasphere.tiles.len());
    ...
    fn test_hexagon_stats_different_subdivisions() {
        let hex1 = Hexasphere::new(1.0, 1, 1.0);
        let hex2 = Hexasphere::new(1.0, 2, 1.0);
    ...
    fn test_hexagon_stats_variation_metrics() {
        ...
        assert!(variation_coefficient < 1.0, "Variation should be reasonable: {}", variation_coefficient);
        ...
        assert!(size_ratio < 3.0, "Size ratio should be reasonable: {}", size_ratio);
    }
    ...
    fn test_hexagon_stats_different_radii() {
        ...
        assert!((radius_ratio - scale_factor).abs() < 0.1, 
            "Radius should scale: {} vs expected {}", radius_ratio, scale_factor);
        
        let edge_ratio = large_stats.average_hexagon_edge_length / small_stats.average_hexagon_edge_length;
        assert!((edge_ratio - scale_factor).abs() < 0.1,
            "Edge length should scale: {} vs expected {}", edge_ratio, scale_factor);
    }
    ...
    fn test_hexagon_stats_consistency() {
        ...
        assert!((stats1.average_hexagon_edge_length - stats2.average_hexagon_edge_length).abs() < 0.0001);
    ...
    fn test_hexagon_stats_realistic_values() {
        ...
        assert!(stats.average_hexagon_radius > 0.1 && stats.average_hexagon_radius < 5.0,
            "Hexagon radius should be reasonable: {}", stats.average_hexagon_radius);
        assert!(stats.average_hexagon_edge_length > 0.1 && stats.average_hexagon_edge_length < 5.0,
            "Edge length should be reasonable: {}", stats.average_hexagon_edge_length);
        assert!(stats.average_hexagon_area > 0.01 && stats.average_hexagon_area < 20.0,
            "Area should be reasonable: {}", stats.average_hexagon_area);
        
        // Standard deviation should be much smaller than the average
        assert!(stats.radius_std_deviation < stats.average_hexagon_radius,
