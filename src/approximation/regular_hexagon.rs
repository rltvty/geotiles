...
mod tests {
    ...
    fn test_generate_vertices_angles() {
        ...
        for (i, vertex) in vertices.iter().enumerate() {
            ...
            assert!(
                (vertex.x - expected_x).abs() < 0.001,
                "Vertex {} x: {} vs expected {}",
                i,
                vertex.x,
                expected_x
            );
            assert!(
                (vertex.y - expected_y).abs() < 0.001,
                "Vertex {} y: {} vs expected {}",
                i,
                vertex.y,
                expected_y
            );
    ...
    fn test_generate_vertices_different_radius() {
        ...
        for (small, large) in small_vertices.iter().zip(large_vertices.iter()) {
            ...
            assert!(
                (large_dist / small_dist - 3.0).abs() < 0.001,
                "Distance ratio should be 3.0: {}",
                large_dist / small_dist
            );
        }
    }
    ...
    fn test_generate_vertices_custom_orientation() {
        ...
        assert!(
            vertices[0].x.abs() < 0.001,
            "X should be ~0: {}",
            vertices[0].x
        );
        assert!(
            (vertices[0].y - radius).abs() < 0.001,
            "Y should be radius: {}",
            vertices[0].y
        );
        assert!(
            vertices[0].z.abs() < 0.001,
            "Z should be ~0: {}",
            vertices[0].z
        );
    }
    ...
    fn test_hexasphere_regular_hexagon_params() {
        ...
        for vertex in &vertices {
            ...
            assert!(
                ...
                "Distance should match radius: {} vs {}",
                distance,
                params.radius
            );
        }
        ...
        assert!(
            (params.radius - tile_radius).abs() < 0.1,
            "Param radius should match tile radius: {} vs {}",
            params.radius,
            tile_radius
        );
    }
    ...
    fn test_regular_hexagon_edge_lengths() {
        ...
        assert!(
            (first_length - radius).abs() < 0.001,
            "Edge length {} should equal radius {}",
            first_length,
            radius
        );
    }
    ...
    fn test_regular_hexagon_area_calculation() {
        ...
        assert!(
            (total_area - expected_area).abs() < 0.001,
            "Calculated area {} should match expected {}",
            total_area,
            expected_area
        );
    }
    ...
    fn test_regular_hexagon_symmetry() {
        ...
        for i in 0..3 {
            ...
            assert!(
                (dist_a - dist_b).abs() < 0.001,
                "Opposite vertices should be equidistant: {} vs {}",
                dist_a,
                dist_b
            );
            ...
            assert!(
                (separation - 2.0 * radius).abs() < 0.001,
                "Opposite vertices should be 2*radius apart: {} vs {}",
                separation,
                2.0 * radius
            );
        }
