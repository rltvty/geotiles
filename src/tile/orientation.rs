...
mod tests {
    ...
    fn test_to_rotation_matrix() {
        ...
        for (i, (&actual, &expected)) in matrix.iter().zip(expected.iter()).enumerate() {
            assert!((actual - expected).abs() < 0.001, 
                "Matrix element {} differs: {} vs {}", i, actual, expected);
        }
    }
    ...
    fn test_to_transform_matrix() {
        ...
        for (i, (&actual, &expected)) in matrix.iter().zip(expected.iter()).enumerate() {
            assert!((actual - expected).abs() < 0.001,
                "Transform matrix element {} differs: {} vs {}", i, actual, expected);
        }
    }
    ...
    fn test_custom_orientation_matrix() {
        ...
        for (i, (&actual, &expected)) in matrix.iter().zip(expected.iter()).enumerate() {
            assert!((actual - expected).abs() < 0.001,
                "Custom matrix element {} differs: {} vs {}", i, actual, expected);
        }
    }
        ...
        for tile in &hexasphere.tiles {
            if let Some(orientation) = tile.get_orientation() {
                ...
                let up_len = (orientation.up.x.powi(2) + 
                             ...
                             orientation.up.z.powi(2)).sqrt();
                let forward_len = (orientation.forward.x.powi(2) + 
                                  orientation.forward.y.powi(2) + 
                                  orientation.forward.z.powi(2)).sqrt();
                
                assert!((right_len - 1.0).abs() < 0.1, "Right vector should be unit length");
                assert!((up_len - 1.0).abs() < 0.1, "Up vector should be unit length");
                assert!((forward_len - 1.0).abs() < 0.1, "Forward vector should be unit length");
    ...
    fn test_orthogonality() {
        ...
        if let Some(orientation) = tile.get_orientation() {
            ...
            assert!(right_up_dot.abs() < 0.1, "Right and Up should be orthogonal");
            assert!(right_forward_dot.abs() < 0.1, "Right and Forward should be orthogonal");
            assert!(up_forward_dot.abs() < 0.1, "Up and Forward should be orthogonal");
        }
    }
