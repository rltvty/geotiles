...
fn test_key_library_claims() {
    ...
    for level in [2, 5, 10] {
        ...
        println!(
            "   Level {}: {} hexagons ({} total - 12 pentagons) ✓",
            level,
            hexagons,
            hs.tiles.len()
        );
    ...
    assert!(
        time_15.as_secs() < 10,
        "Should complete level 15 in under 10 seconds"
    );
...
fn test_user_reported_tile_counts() {
    ...
    assert_eq!(
        hs_20.tiles.len(),
        4002,
        "User reported level 20 should have 4002 tiles"
    );
    ...
    assert_eq!(
        expected_30, 9002,
        "Level 30 should have 9002 tiles by formula"
    );
...
fn test_shape_instancing_benefits() {
    ...
    for (level, min_expected_benefit) in levels_and_expected_benefits {
        ...
        println!(
            "Level {}: {} tiles, ~{} unique shapes = {:.1}x theoretical benefit",
            level,
            hs.tiles.len(),
            unique_shapes_estimate,
            actual_benefit
        );

        assert!(
            actual_benefit >= min_expected_benefit,
            "Should achieve at least {}x benefit at level {}",
            min_expected_benefit,
            level
        );
    }
}
...
fn test_corrected_documentation_examples() {
    ...
    for (level, expected) in doc_examples {
        if level <= 15 {
            // Only test reasonable levels in CI
            let hs = Hexasphere::new(1.0, level, 1.0);
            assert_eq!(
                hs.tiles.len(),
                expected,
                "Documentation example level {} should have {} tiles",
                level,
                expected
            );
        } else {
            ...
            assert_eq!(
                formula_result, expected,
                "Formula should give {} tiles for level {}",
                expected, level
            );
        }
    }
...
fn test_user_intended_workflow() {
    ...
    println!(
        "Level 20 hexagons: {}",
        hs_20.tiles.iter().filter(|t| t.boundary.len() == 6).count()
    );
    println!(
        "Level 20 pentagons: {}",
        hs_20.tiles.iter().filter(|t| t.boundary.len() == 5).count()
    );

    // Should be practical for real-time use
    assert!(
        time_20.as_secs() < 30,
        "Level 20 should generate in under 30 seconds"
    );
    assert_eq!(
        hs_20.tiles.len(),
        4002,
        "Should match user's reported count"
    );
    ...
    assert!(
        ...
        "Should have valid orientations for rendering"
    );
