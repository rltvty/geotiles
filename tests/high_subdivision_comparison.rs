...
fn test_subdivision_22_comparison() {
    ...
    println!(
        "Traditional generation completed in: {:?}",
        traditional_time
    );
    ...
    for instance in &shape_data.instances {
        ...
        assert!(
            instance.shape_index < shape_data.shapes.len(),
    ...
    println!(
        "  Traditional vertex data: {} KB",
        vertex_data_traditional / 1024
    );
    println!(
        "  Instanced vertex data: {} KB",
        vertex_data_instanced / 1024
    );
    ...
    assert!(
        memory_savings > 5.0,
        "Should achieve significant memory savings at level 22"
    );

    println!(
        "✅ High subdivision level {} validation complete",
        subdivision
    );
}
...
fn benchmark_requested_levels() {
    ...
    for level in levels {
        ...
        println!(
            "  Instanced: {} shapes with {} instances",
...
fn test_bevy_integration_workflow() {
    ...
    println!("  Unique shapes: {}", shape_data.shapes.len());
    println!(
        "  GPU draw calls needed: {} (instead of {})",
        shape_data.shapes.len(),
        hexasphere.tiles.len()
    );
    ...
    for (shape_idx, shape) in shape_data.shapes.iter().enumerate() {
        ...
        let shape_instances = shape_data
            .instances
            .iter()
    ...
    assert!(
        ...
        "Instanced approach should be more memory efficient"
    );

    println!("\n✅ Bevy integration workflow validated");
    println!(
        "Ready for GPU instancing with {:.1}x compression!",
