...
fn test_subdivision_35_comparison() {
    ...
    println!(
        "Expected tiles by formula: {} tiles",
    ...
    println!(
        "Shape instances: {}",
        format_number(shape_data.instances.len())
    );
    ...
    println!(
        "⚡ Analysis speedup: {:.1}x faster than generation",
        speedup
    );
    ...
    println!(
        "📍 Maximum center position error (sample): {:.9}",
        max_center_error
    );
    assert!(
        max_center_error < 0.001,
        "Center positions should match within tolerance"
    );

    // Memory efficiency estimate
    let traditional_memory_mb = (hexasphere.tiles.len() * 200) / (1024 * 1024); // ~200 bytes per tile
    let instanced_memory_mb =
        (shape_data.shapes.len() * 100 + shape_data.instances.len() * 64) / (1024 * 1024);
    ...
    println!("\n🎮 Bevy Rendering Implications:");
    println!(
        "Traditional: {} separate meshes to render",
        format_number(hexasphere.tiles.len())
    );
    println!(
        "Instanced: {} unique meshes with ~{} instances each (avg)",
    ...
    println!(
        "• Level 22: 4,842 tiles → Level 35: {} tiles",
        format_number(hexasphere.tiles.len())
    );
    println!(
        "• That's a {:.1}x increase in complexity!",
        hexasphere.tiles.len() as f64 / 4842.0
    );
    println!(
        "• But still ~{:.1}x compression with shape instancing!",
        compression
    );
}
