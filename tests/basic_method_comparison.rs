...
fn test_requested_subdivision_levels_basic() {
    ...
    for level in quick_levels {
        ...
        assert_eq!(hexasphere.tiles.len(), expected_tiles,
            "Tile count should match formula for level {}", level);
        ...
        assert!(valid_orientations > 0, "Should have some valid orientations");
...
fn test_generation_consistency() {
    ...
    for hex_size in hex_sizes {
        ...
        let pentagons = hexasphere.tiles.iter().filter(|t| t.boundary.len() == 5).count();
        assert_eq!(pentagons, 12, "Pentagon count should be independent of hex_size");
        
        println!("hex_size {}: {} tiles, 12 pentagons ✓", hex_size, hexasphere.tiles.len());
    }
    ...
    for radius in radii {
        ...
        let max_distance = hexasphere.tiles.iter()
            .map(|t| (t.center_point.x.powi(2) + t.center_point.y.powi(2) + t.center_point.z.powi(2)).sqrt())
            .fold(0.0f64, |a, b| a.max(b));
        
        assert!((max_distance - radius).abs() < 0.1,
            "Max distance should be approximately equal to radius");
        
        println!("radius {}: {} tiles, max distance {:.3} ✓", 
            radius, hexasphere.tiles.len(), max_distance);
    }
...
fn test_geometric_properties() {
    ...
    println!("Testing {} tiles at subdivision {}", hexasphere.tiles.len(), level);
    ...
    for (i, tile) in hexasphere.tiles.iter().enumerate() {
        ...
        let center_distance = (tile.center_point.x.powi(2) + 
                              ...
                              tile.center_point.z.powi(2)).sqrt();
        assert!((center_distance - 1.0).abs() < 0.1,
        ...
        for vertex in &tile.boundary {
            ...
            assert!(vertex_distance > 0.5 && vertex_distance < 1.5,
                "Boundary vertices should be near unit sphere");
        }
    }
    
    assert_eq!(invalid_tiles, 0, "All tiles should be pentagons or hexagons");
...
fn test_neighbor_relationships() {
    ...
    for tile in &hexasphere.tiles {
        ...
        for &neighbor_idx in &tile.neighbors {
            assert!(neighbor_idx < hexasphere.tiles.len(),
                "Neighbor index {} should be valid", neighbor_idx);
        }
    }
...
fn test_performance_scaling() {
    ...
    for level in test_levels {
        ...
        println!("{:5} | {:6} | {:8?} | {:9.1}x | {:8.0}",
            level, hexasphere.tiles.len(), generation_time, time_ratio, tiles_per_sec);
...
fn test_shape_instancing_readiness() {
    ...
    println!("Analyzing subdivision {} for shape instancing potential", level);
    ...
    println!("Regular hexagon approximations: {}", hex_approximations.len());
    
    // This should be very close to the number of hexagons
    assert!((hex_approximations.len() as i32 - hexagons as i32).abs() <= 1,
