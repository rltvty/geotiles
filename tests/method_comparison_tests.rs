...
fn test_traditional_vs_instancing_identical_output() {
    ...
    for subdivision in test_levels {
        println!("\n=== Comparing Methods at Subdivision Level {} ===", subdivision);
        ...
        for instance in &shape_data.instances {
            assert!(instance.tile_index < hexasphere.tiles.len(),
                "Tile index {} should be valid", instance.tile_index);
        ...
        for instance in &shape_data.instances {
            ...
            assert_eq!(shape.sides, original_tile.boundary.len(),
                "Shape sides must match original tile boundary length");
        }
        
        // Validation 4: Pentagon and hexagon counts match
        let original_pentagons = hexasphere.tiles.iter()
            .filter(|t| t.boundary.len() == 5).count();
        let original_hexagons = hexasphere.tiles.iter()
            .filter(|t| t.boundary.len() == 6).count();
        
        let instanced_pentagons = shape_data.instances.iter()
            .filter(|i| shape_data.shapes[i.shape_index].sides == 5).count();
        let instanced_hexagons = shape_data.instances.iter()
            .filter(|i| shape_data.shapes[i.shape_index].sides == 6).count();
        
        assert_eq!(original_pentagons, instanced_pentagons,
            "Pentagon count must match between methods");
        assert_eq!(original_hexagons, instanced_hexagons,
        ...
        for instance in &shape_data.instances {
            ...
            assert!(distance < 0.001,
                "Center positions should match within tolerance (distance: {})", distance);
        }
        ...
        for instance in &shape_data.instances {
            ...
            let right_len = (instance.orientation.right.x.powi(2) +
                           ...
                           instance.orientation.right.z.powi(2)).sqrt();
            let up_len = (instance.orientation.up.x.powi(2) +
                         instance.orientation.up.y.powi(2) +
                         instance.orientation.up.z.powi(2)).sqrt();
            let forward_len = (instance.orientation.forward.x.powi(2) +
                             instance.orientation.forward.y.powi(2) +
                             instance.orientation.forward.z.powi(2)).sqrt();
            
            assert!((right_len - 1.0).abs() < 0.01, "Right vector should be normalized");
            assert!((up_len - 1.0).abs() < 0.01, "Up vector should be normalized"); 
            assert!((forward_len - 1.0).abs() < 0.01, "Forward vector should be normalized");
        }
...
fn test_reconstructed_geometry_accuracy() {
    ...
    println!("\n=== Testing Geometry Reconstruction at Level {} ===", subdivision);
    ...
    for instance in &shape_data.instances {
        ...
        let center_error = ((original_tile.center_point.x - instance.center.x).powi(2) +
                           ...
                           (original_tile.center_point.z - instance.center.z).powi(2)).sqrt();
        ...
        assert_eq!(shape.vertices.len(), original_tile.boundary.len(),
        ...
        for vertex in &shape.vertices {
            ...
            assert!(vertex_magnitude > 0.001 && vertex_magnitude < 10.0,
...
fn test_requested_subdivision_levels() {
    ...
    for (subdivision, description) in test_cases {
        println!("\n=== Testing {} (subdivision {}) ===", description, subdivision);
        ...
        let compression = hexasphere.tiles.len() as f64 / shape_data.shapes.len() as f64;
        println!("Tiles: {}, Shapes: {}, Compression: {:.1}x", 
...
fn test_hexagon_only_filtering() {
    ...
    println!("\n=== Testing Hexagon-Only Filtering at Level {} ===", subdivision);
    ...
    println!("All shapes: {} shapes, {} instances", 
        all_shapes.shapes.len(), all_shapes.instances.len());
    println!("Hex-only shapes: {} shapes, {} instances", 
        hex_shapes.shapes.len(), hex_shapes.instances.len());
    
    // Validate hexagon-only results
    let expected_hexagons = hexasphere.tiles.iter()
        .filter(|t| t.boundary.len() == 6).count();
    
    assert_eq!(hex_shapes.instances.len(), expected_hexagons,
    ...
    for instance in &hex_shapes.instances {
        let original_tile = &hexasphere.tiles[instance.tile_index];
        assert_eq!(original_tile.boundary.len(), 6,
            "All instances should reference hexagonal tiles");
    }
...
fn test_uniform_shape_approach() {
    ...
    println!("\n=== Testing Uniform Shape Approach at Level {} ===", subdivision);
    
    let hexasphere = Hexasphere::new(1.0, subdivision, 1.0);
    let (uniform_shape, instances) = hexasphere.get_uniform_shape_instances();
    
    println!("Uniform shape: {} sides, radius {:.4}", 
    ...
    for instance in &instances {
        assert_eq!(instance.shape_index, 0, "All instances should reference shape 0");
    }
    
    // Should have instance for every tile with valid orientation
    let tiles_with_orientation = hexasphere.tiles.iter()
        .filter(|t| t.get_orientation().is_some()).count();
    
    assert_eq!(instances.len(), tiles_with_orientation,
        "Should have instance for every tile with valid orientation");
...
fn test_performance_comparison() {
    ...
    for subdivision in [6, 10, 12] {
        ...
        assert!(instancing_time <= traditional_time,
