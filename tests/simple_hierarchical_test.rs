...
fn test_simple_hierarchical_approximation() {
    ...
    let natural_hexagon_count = natural_shapes
        .shapes
        .iter()
        .filter(|s| s.sides == 6)
        .count();
    let natural_pentagon_count = natural_shapes
        .shapes
        .iter()
        .filter(|s| s.sides == 5)
        .count();
    ...
    let (simplified_shapes, simplified_instances, stats) =
        hexasphere.get_simplified_shapes(target_shapes, tolerance);
    ...
    println!(
        "  Pentagon shapes: {} (always preserved)",
        simplified_pentagon_count
    );
    println!("  Total shapes: {}", simplified_shapes.len());
    println!(
        "  Hexagon compression: {:.1}x ({} → {})",
        stats.compression_ratio, stats.original_shape_count, stats.simplified_shape_count
    );
    println!(
        "  Average error (hexagons): {:.2}%",
        stats.average_error * 100.0
    );
    println!("  Max error (hexagons): {:.2}%", stats.max_error * 100.0);
    println!(
        "  Pentagon instances preserved: {}",
        stats.pentagon_shapes_preserved
    );

    // Validate results
    assert_eq!(
        simplified_instances.len(),
        hexasphere.tiles.len(),
        "Should have same number of instances as tiles"
    );
    assert!(
        simplified_hexagon_count <= target_shapes + 5,
