...
use geotiles::hexasphere::shape_instances::{ShapeInstanceData, TileShape, TileInstance};
...
fn test_shape_instances_preserve_geometry() {
    for subdivision in 2..=6 {
        ...
        assert_eq!(shape_data.instances.len(), hexasphere.tiles.len(),
            "Instance count should match tile count at subdivision {}", subdivision);
...
fn test_unique_shape_count_pattern() {
    ...
    for (subdivision, expected) in expected_counts {
        ...
        assert_eq!(shape_data.shapes.len(), expected,
            "Subdivision {} should have {} unique shapes", subdivision, expected);
    }
}
...
fn test_complete_tile_coverage() {
    for subdivision in 2..=5 {
        ...
        for i in 0..hexasphere.tiles.len() {
            assert!(covered_tiles.contains(&i),
                "Tile {} should be covered by an instance", i);
        }
        
        // Verify no duplicate coverage
        assert_eq!(covered_tiles.len(), shape_data.instances.len(),
...
fn test_shape_type_classification() {
    for subdivision in 2..=5 {
        ...
        assert!(pentagon_shapes >= 1, "Should have at least 1 pentagon shape");
        
        // Most shapes should be hexagons (except for subdivision 1)
        if subdivision > 1 {
            assert!(hexagon_shapes > pentagon_shapes, 
...
fn test_hexagon_only_shapes() {
    for subdivision in 3..=6 {
        ...
        let expected_hexagons = hexasphere.tiles.iter()
            ...
            .count();
        assert_eq!(hex_data.instances.len(), expected_hexagons,
...
fn test_compression_ratios() {
    for subdivision in 3..=8 {
        ...
        assert!(compression_ratio >= 2.0,
            "Should achieve at least 2x compression at subdivision {}", subdivision);
        
        if subdivision >= 5 {
            assert!(compression_ratio >= 5.0,
                "Should achieve at least 5x compression at subdivision {}", subdivision);
        }
        
        println!("Subdivision {}: {} tiles → {} shapes = {:.1}x compression",
            subdivision, hexasphere.tiles.len(), shape_data.shapes.len(), compression_ratio);
    }
}
...
fn test_shape_analysis_consistency() {
    for subdivision in 2..=6 {
        ...
        assert_eq!(shape_data.shapes.len(), analyzer.unique_shape_count(),
