...
fn test_final_comparison_6_12_22() {
    ...
    for level in test_levels {
        ...
        assert_eq!(
            ...
            expected_tiles,
            "Tile count should match formula 10n² + 2 for level {}",
            level
        );
        ...
        let shape_pentagons = shape_data
            .instances
            .iter()
            .filter(|i| shape_data.shapes[i.shape_index].sides == 5)
            .count();
        let shape_hexagons = shape_data
            .instances
            .iter()
        ...
        println!(
            "✅ Level {} validation complete - methods produce identical output!\n",
            level
        );
    }
