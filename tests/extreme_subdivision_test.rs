...
fn test_extreme_subdivision_46() {
    ...
    let gpu_savings =
        vertex_data_traditional as f64 / (vertex_data_instanced + instance_data) as f64;
    ...
    println!("\n🤓 Fun Facts:");
    println!(
        "• If each tile was 1cm², this sphere would have {:.1} m² surface area",
        hexasphere.tiles.len() as f64 / 10000.0
    );
    println!(
        "• That's about the size of a {} basketball court!",
        if hexasphere.tiles.len() > 4000000 {
            "large"
        } else {
            "small"
        }
    ...
    println!(
        "• You reduced {} individual meshes down to just {} unique shapes!",
        format_number(hexasphere.tiles.len()),
        shape_data.shapes.len()
    );
}
